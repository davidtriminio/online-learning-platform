...
import { toRegisterRequestDto } from './mappers/register.mapper'
...
export class AuthRepository {
  ...
  register(form: RegisterForm): Observable<AuthUser> {
    return this.http
      .post<ApiResponse<UserResponseDto>>(
        `${this.baseUrl}${AUTH_ENDPOINTS.addNewUser}`,
        toRegisterRequestDto(form),
      )
      .pipe(map((res) => toAuthUser(res.data)))
  }
}
